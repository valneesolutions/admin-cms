...
function Check({ passed, children }: { passed: boolean; children: ReactNode }) {
  return <li className={passed ? 'blog-seo-health__pass' : 'blog-seo-health__warning'}>{children}</li>
}
...
export function BlogSEOHealth() {
  ...
  return (
    <section className="blog-seo-health">
      ...
      <div className="blog-seo-health__meter" aria-label={`SEO health score: ${score}%`}>
        <span style={{ width: `${score}%` }} />
      </div>
      <ul>
        <Check passed={words >= 300}>{words} words {words >= 300 ? '— good' : '— aim for 300+'}</Check>
        <Check passed={Boolean(slug)}>{slug ? `/${slug}` : 'Add a URL slug'}</Check>
        <Check passed={displayedTitle.length >= 40 && displayedTitle.length <= 60}>Title: {displayedTitle.length}/60 characters</Check>
        <Check passed={displayedDescription.length >= 120 && displayedDescription.length <= 160}>Description: {displayedDescription.length}/160 characters</Check>
      </ul>
    </section>
  )
}
